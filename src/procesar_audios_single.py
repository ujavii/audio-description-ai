...
# Configuración
# ========================
AUDIO_PATH = ""  # si vacío, toma el primero en AUDIOS_DIR
...
try:
    ...
    try:
        ...
        raise ImportError("No se encontró ni Faster Whisper ni Whisper. Instala: "
                          "pip install faster-whisper  (y opcionalmente) pip install openai-whisper")
...
def natural_sort(lista):
    return sorted(lista, key=_natural_key)
...
def _append_fila_csv(fila, campos):
    with open(_ruta_salida_csv(), "a", encoding="utf-8-sig", newline="") as f:
        csv.DictWriter(f, fieldnames=campos, delimiter=';').writerow(fila)

# ========================
# Trim silencio (opcional)
# ========================
...
def _transcribir_intentos_faster(model, ruta_audio):
    ...
    for i, (desc, kwargs) in enumerate(intentos, start=1):
        try:
            ...
            if texto:
                print(f"   Transcripción con '{desc}'"); return texto
...
def _transcribir_intentos_whisper(modelo, ruta_audio):
    ...
    for i, (desc, kwargs) in enumerate(intentos, start=1):
        try:
            ...
            if texto:
                print(f"   Transcripción Whisper '{desc}'"); return texto
            print("   Texto vacío; probando siguiente…")
        except Exception as e:
...
def transcribir_audio(ruta_audio):
    ...
    if FASTER_WHISPER_AVAILABLE:
        ...
        try:
            ...
            if texto:
                return texto
            print("   FW sin texto; probando Whisper normal…")
        except Exception as e:
...
if __name__ == "__main__":
    ...
    if AUDIO_PATH and os.path.isfile(AUDIO_PATH):
        ...
        lista = [f for f in os.listdir(str(AUDIOS_DIR)) if f.lower().endswith((".mp3", ".wav", ".m4a", ".flac"))]
        if not lista:
            raise SystemExit(f"No hay audios en: {AUDIOS_DIR}")
        ruta = os.path.join(str(AUDIOS_DIR), natural_sort(lista)[0])
    ...
    try:
        ...
        dt_trans = time.perf_counter() - t0
        print(f"Transcripción: {_fmt_hms(time.perf_counter() - t0)}")

        # Técnicos
        fila.update(extraer_metadatos_tecnicos(ruta))

        dt_api = 0.0
        if texto:
            t1 = time.perf_counter()
            meta = extraer_metadatos_api(texto)
            dt_api = time.perf_counter() - t1
            print(f"Descripción (API): {_fmt_hms(dt_api)}")
        ...
        dt_save = time.perf_counter() - t2

        print(f"Tiempo total: {_fmt_hms(time.perf_counter() - t_all)}")
        print(f"Guardado en CSV: {nombre}")
        print(f"CSV: {_ruta_salida_csv()}")
    except Exception as e:
